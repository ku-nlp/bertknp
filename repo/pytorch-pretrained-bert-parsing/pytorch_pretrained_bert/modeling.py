...
try:
    ...
    print("Better speed can be achieved with apex installed from https://www.github.com/nvidia/apex.", file=sys.stderr)
...
class PreTrainedBertModel(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, num_expand_vocab=None,
                        *inputs, **kwargs):
        """
        ...
        if num_expand_vocab is not None:
            model.bert.expand_vocab(num_expand_vocab)
    ...
    def expand_vocab(self, num_expand_vocab):
        ...
        old_word_embeddings_numpy = self.embeddings.word_embeddings.weight.detach().numpy()
        vocab_size = self.embeddings.word_embeddings.weight.shape[0]
        new_word_embeddings = nn.Embedding(vocab_size + num_expand_vocab,
        ...
        self.embeddings.word_embeddings = new_word_embeddings
...
class BertForParsing(PreTrainedBertModel):
    """BERT model for Parsing (head selection).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    def __init__(self, config, parsing_algorithm="zhang",
        ...
        if self.token_label_vocabulary is not None:
            for namespace in self.token_label_vocabulary:
                if namespace == "dep_label":
                    if self.parsing_algorithm == "biaffine":
                        self.head_tag_linear = nn.Linear(config.hidden_size, tag_representation_dim)
                        self.child_tag_linear = nn.Linear(config.hidden_size, tag_representation_dim)
                        
                        self.dep_label_classifier = nn.Linear(tag_representation_dim * 2, token_label_vocabulary[namespace].num_label)
                        self.dep_label_bilinear = nn.Bilinear(tag_representation_dim, tag_representation_dim, token_label_vocabulary[namespace].num_label)
                    elif self.parsing_algorithm == "zhang":                        
                        # target embedding + head embedding -> hidden_size
                        self.dep_label_classifier_l1 = nn.Linear(config.hidden_size * 2, config.hidden_size)
                        self.dep_label_classifier_l2 = nn.Linear(config.hidden_size, token_label_vocabulary[namespace].num_label)                    
                else:
                    setattr(self, "tag_classifier_" + namespace, nn.Linear(config.hidden_size, token_label_vocabulary[namespace].num_label))
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, heads=None, arguments_set=None, ng_arg_ids_set=None, token_tags=None):
        ...
        if self.parsing_algorithm == "biaffine":
            ...
            # [batchsize, 1, sequence_length]
            bias = self.arc_b(h_j).squeeze(2).unsqueeze(1)
        ...
        if self.pas_analysis is False:
            seq_length = input_ids.size(1)
            eye_mask = torch.eye(seq_length, device=input_ids.device).to(dtype=next(self.parameters()).dtype) * -10000.0
    ...
    def get_dep_labels(self, sequence_output, heads, dep_labels=None):
        batch_size = sequence_output.size(0)
        # [batchsize, sequence_length, hidden_size]
        head_embeddings = sequence_output[torch.arange(batch_size, dtype=torch.long, device=sequence_output.device).unsqueeze(1), heads]

        if self.parsing_algorithm == "biaffine":
            head_tag_embeddings = torch.relu(self.head_tag_linear(head_embeddings))
            child_tag_embeddings = torch.relu(self.child_tag_linear(sequence_output))

            dep_label_logits_bilinear = self.dep_label_bilinear(child_tag_embeddings, head_tag_embeddings)
            dep_label_logits_linear = self.dep_label_classifier(torch.cat([child_tag_embeddings, head_tag_embeddings], -1))
            dep_label_logits = dep_label_logits_bilinear + dep_label_logits_linear
        elif self.parsing_algorithm == "zhang":                                    
            dep_label_logits = self.dep_label_classifier_l2(torch.tanh(self.dep_label_classifier_l1(torch.cat([sequence_output, head_embeddings], -1))))

        if dep_labels is not None:
            loss_fct = CrossEntropyLoss(ignore_index=-1)
            return loss_fct(dep_label_logits.view(-1, dep_label_logits.size(-1)), dep_labels.view(-1))
        else:
            return dep_label_logits

    def get_topk_dep_labels(self, sequence_output, topk_heads):
        batch_size = sequence_output.size(0)
        sequence_length = sequence_output.size(1)
        
        batch_idx = torch.arange(batch_size).view(batch_size, 1, 1).expand(-1, sequence_length, self.num_topk_heads)
        len_idx = torch.arange(sequence_length).view(1, sequence_length, 1).expand(batch_size, -1, self.num_topk_heads)

        # [batch_size, sequence_length, topk, hidden_size]
        target_embeddings = sequence_output.unsqueeze(2).expand(-1, -1, self.num_topk_heads, -1)

        # [batch_size, sequence_length, topk, hidden_size]
        head_embeddings = sequence_output.unsqueeze(1).expand(-1, sequence_length, -1, -1)[batch_idx, len_idx, topk_heads, :]

        if self.parsing_algorithm == "biaffine":
            target_tag_embeddings = self.child_tag_linear(target_embeddings)
            head_tag_embeddings = self.head_tag_linear(head_embeddings)

            topk_dep_label_logits = self.dep_label_bilinear(target_tag_embeddings, head_tag_embeddings) + \
                                    self.dep_label_classifier(torch.cat([target_tag_embeddings, head_tag_embeddings], -1))
            
        elif self.parsing_algorithm == "zhang":                                                
            # [batch_size, sequence_length, topk, num_label]
            topk_dep_label_logits = self.dep_label_classifier_l2(torch.tanh(self.dep_label_classifier_l1(torch.cat([target_embeddings, head_embeddings], -1))))
        # [batch_size, sequence_length, topk]
        _, topk_dep_labels = torch.max(topk_dep_label_logits, dim=3)
        
        return topk_dep_labels
