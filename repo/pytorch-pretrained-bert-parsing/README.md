# PyTorch Pretrained Bert for Parsing
...
- Specify your output directotry `--output_dir /somewhere` when you run `run_parsing`.
```bash
$ export CUDA_VISIBLE_DEVICES="0,1"
$ cd examples
$ python run_parsing.py --train_file /orange/kawahara/work/parse-eval/data/kyoto/latest/conllu/train/kc-train.conllu --bert_model /larch/shibata/bert/preprocess/181123_subword_wikipedia/pretraining_model_10e --output_dir /somewhere --do_train --max_seq_length 192 --predict_file /orange/kawahara/work/parse-eval/data/kyoto/latest/conllu/test/kc-test.conllu --do_predict --train_batch_size 16 --parsing
```

### 3. Evaluation
```bash
$ python /somewhere/evaluation_script/conll17_ud_eval.py -v /orange/kawahara/work/parse-eval/data/kyoto/latest/conllu/test/kc-test.conllu /somewhere/predictions.txt
```

By using the following makefile, you can run both model training/testing and evaluation.
```bash
$ export CUDA_VISIBLE_DEVICES="0,1"
$ cd examples
$ make -f Makefile.run_parsing PARSING=1 PRETRAINED_MODEL_DIR=/larch/shibata/bert/preprocess/181213_subword_wikipedia/pretraining_model_20e TASK_NAME=181213_subword_wikipedia_20e
```
- OUTPUT_DIR_BASENAME=(str): output directory
- TASK_NAME=(str): for a directory basename
    - A trained model and result are outputted under `$(OUTPUT_DIR_BASENAME)/$(TASK_NAME)`
- WORD_SEGMENTATION=1: perform word segmentation
- POS_TAGGING=1: perform pos tagging
- PARSING=1: perform parsing
- ESTIMATE_DEP_LABEL=1: estimate dependency labels
- PRETRAINED_MODEL_DIR=(str): specify a pretrained model
- USE_GOLD_WORD_SEGMENTATION_IN_TEST=1: use gold word segmentations in testing (corresponds to normal parsing) 

#### Examples
- CTB-5
```bash
$ make all -f Makefile.run_parsing PRETRAINED_MODEL_DIR=/larch/share/bert/google_pretrained_models/chinese_L-12_H-768_A-12 TASK_NAME=chinese_ctb51j_pos WORD_SEGMENTATION=1 MAX_SEQ_LENGTH=448 LANG=zh TRAIN_BATCH_SIZE=8 PREDICT_BATCH_SIZE=4 POS_TAGGING=1
```

- CTB-7
```bash
$ make all -f Makefile.run_parsing PRETRAINED_MODEL_DIR=/larch/share/bert/google_pretrained_models/chinese_L-12_H-768_A-12 TASK_NAME=chinese_ctb7_pos WORD_SEGMENTATION=1 MAX_SEQ_LENGTH=448 LANG=zh TRAIN_BATCH_SIZE=8 PREDICT_BATCH_SIZE=4 POS_TAGGING=1 TRAIN_FILE=/share/text/CTB_Dependency/ctb7-train.conllu PREDICT_FILE=/share/text/CTB_Dependency/ctb7-test.conllu
```
- UD_Chinese
```bash
$ make all -f Makefile.run_parsing PRETRAINED_MODEL_DIR=/larch/share/bert/google_pretrained_models/chinese_L-12_H-768_A-12 TASK_NAME=CHINESE_USD_test WORD_SEGMENTATION=1 POS_TAGGING=1 SUBPOS_TAGGING=1 FEATS_TAGGING=1 MAX_SEQ_LENGTH=256 TRAIN_FILE=/larch/shibata/UD/ud-treebanks-v2.2/UD_Chinese-GSD/zh_gsd-ud-train.conllu PREDICT_FILE=/larch/shibata/UD/result/UD_Chinese-GSD/ete/zh_gsd.test.pred.ete.conllu ESTIMATE_DEP_LABEL=1 UD_EVAL_FILE=conll18_ud_eval.py GOLD_FILE=/larch/shibata/UD/ud-treebanks-v2.2/UD_Chinese-GSD/zh_gsd-ud-test.conllu
```
